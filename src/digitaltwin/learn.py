"""
A model investigator class for streaming with ROSE.

This file is a DRAFT.....

It automatically batches input coming in, and fires the learner's workflow on
each batch input.

How to use:

Subclass StreamingLearnerInvestigator:


class MyInvestigator(StreamingLearnerInvestigator):
    def __init__(...):
        super().__init__(...)

        self.learner = ....

    async def on_batch(batch_of_data: list):
        pass

    async def run_workflow(batch_of_data: list):
        ....
        return model, metrics    # return None for no model
        ....

    async def main_loop(runtime):
        super().main_loop(runtime)



It will automatically call run_workflow when batch arrives.
"""
...
from rose.al.active_learner import Learner  # type: ignore
...
class StreamingLearnerInvestigator(ModelInvestigator):
    """A `ModelInvestigator` that adds basic streaming to ROSE."""
    ...
    def __init__(
        ...
        self._learn_backend = learn_backend
        self._batch_size = batch_size
        self._max_dataset_size = max_dataset_size

        self._data_update = asyncio.Event()
        self._dataset: list[Any] = []
        self._batch_values: list[Any] = []
        self._buff_counter = 0

        self.inference_task: Optional[Callable] = None

    async def _on_input_cb(self, in_data: TypedData):
        # add the data to large database
        self._dataset.append(in_data.data)

        if len(self._dataset) > self._max_dataset_size:
            self._dataset.pop(0)

        # workflow currently running. Don't trigger batch:
        self._buff_counter += 1

        if self._buff_counter >= self._batch_size:
            self._buff_counter = 0
            await self.on_batch(self._dataset[-1 * self._batch_size :])
            self._data_update.set()
            # trigger workflow.

    # -- what subclasses shape ----------------------------------------------

    async def on_batch(batch_of_data: list):
        """Called every time a new batch is ready"""
        pass

    async def run_workflow(batch_of_data: list):
        """Called every time a new batch is ready"""
        return None  # or return model, metrics
    ...
    async def main_loop(self, runtime: RuntimeAPI):
        ...
        runtime.set_inference_task(self.inference_task)
        runtime.subscribe_to_topic(RuntimeAPI.ON_INPUT, self._on_input_cb)
        runtime.publish_new_model(*self.bootstrap_model())

        while True:
            await self._data_update.wait()
            self._batch_values = self._dataset[-1 * self._batch_size]
            result = await self.run_workflow(self._batch_values)
            ...
            runtime.publish_new_model(model, metrics)

    # Later to be superseded by Issue #27 ? .... default backend config. User
