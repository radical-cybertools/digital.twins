...
ENGINES = {'engines': {
    ...
    'learning': {'endpoint_name': LEARNING_ENDPOINT, 'backends': ['concurrent']},
}}
```
...
- **`'task'`** runs the twin's components, including the inference task.
  ...
  co-located endpoint.
- **`'learning'`** runs the learner's training, active-learning and
...
`'learning'` is optional.  Left out of the config it aliases
`'inference'`, so
...
not survive a `'learning'` endpoint that shares no filesystem with the
...
RADICAL_ORBIT_RHAPSODY_BACKEND=concurrent \
radical-orbit-endpoint.py -n dt_learning_ep

# 4 - the client
cd test/10-learner
DT_INFERENCE_ENDPOINT=dt_inference_ep DT_LEARNING_ENDPOINT=dt_learning_ep \
python run_me.py
```
